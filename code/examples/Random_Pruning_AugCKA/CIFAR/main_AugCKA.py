...
from models import resnet_CKA, initializers, vgg
    ...
    def forward(self, blocks):
        ...
        for i in range(len(blocklist)-1):
            for j in range(i+1, len(blocklist)):
                ...
                losssum += linear_CKA(X, Y)
        # import pdb;
        # pdb.set_trace()
        loss = 2*losssum/len(blocklist)/(len(blocklist)-1)
...
def train(args, model, device, train_loader, optimizer, epoch, mask=None):
    ...
    correct = 0
    n = 0
    cka_criterion, weight = CKALoss(), args.cka_weight
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output, featlist = model(data)
        # import pdb; pdb.set_trace()

        loss = F.nll_loss(output, target) + weight * cka_criterion(featlist)[0]
...
def main():
    ...
    parser.add_argument('--cka_weight', type=float, default=0.0001)
    # ITOP settings
    ...
    for i in range(args.iters):
        ...
        if 'vgg' in args.model:
            ...
            model = resnet_CKA.Model.get_model_from_name(args.model, initializers.initializations(init_type, args.density), outputs=output).to(device)
