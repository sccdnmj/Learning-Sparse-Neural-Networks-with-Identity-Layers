...
def add_parser_arguments(parser):
    ...
    parser.add_argument('--grow-switch', default='', type=str,
                           help='flag to switch another grow initialization')
    parser.add_argument('--cka-weight', default=0.0001, type=float,
                        help='CKA weight')
        ...
        for feats in blocks:
            ...
            for i in range(_num):
                for j in range(_num):
                    ...
                    if abs(i-j) == 1:
                        hsic_matrix[cnt, 1] += 3 * self._HSIC(K, L)
                    else:
                        hsic_matrix[cnt, 1] += self._HSIC(K, L)
                    # if abs(i-j) == 0:
                    #     hsic_matrix[cnt, 1] += self._HSIC(K, L)
                    # else:
                    #     hsic_matrix[cnt, 1] += 3.0 / float(abs(i-j)) * self._HSIC(K, L)
