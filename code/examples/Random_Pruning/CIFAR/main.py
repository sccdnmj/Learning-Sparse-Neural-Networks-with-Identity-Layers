...
def train(args, model, device, train_loader, optimizer, epoch, mask=None):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output, featlist = model(data, args.samples_to_cal)

        if args.batches_to_cal != 1 and batch_idx%args.batches_to_cal != 0:
            loss = F.nll_loss(output, target)
        else:
            loss = F.nll_loss(output, target) + weight * cka_criterion(featlist)
...
def main():
    ...
    parser.add_argument('--cka_weight', type=float, default=0.0001, help='The hyperparameter beta of CKA-SR')
    parser.add_argument('--batches-to-cal', default=5, type=int,
                        help='Calculate CKA-SR once out of several batches, the default setting is to calculate for each batch')
    parser.add_argument('--samples-to-cal', default=8, type=int,
                        help='Calculate CKA-SR with several samples of each batch, the default setting is to calculate with all the samples')
    ...
    for i in range(args.iters):
        ...
        for epoch in range(1, args.epochs*args.multiplier + 1):
            ...
            if val_acc > best_acc:
                ...
                }, filename=os.path.join(save_subfolder, str(args.cka_weight)+'_model_final.pth'))

            print_and_log('Current learning rate: {0}. Time taken for epoch: {1:.2f} seconds.\n'.format(optimizer.param_groups[0]['lr'], time.time() - t0))
        print('Testing model')
        model.load_state_dict(torch.load(os.path.join(save_subfolder, str(args.cka_weight)+'_model_final.pth'))['state_dict'])
